...
reg <- ranger(mpg ~ ., data = mtcars)
binary <- ranger(I(as.factor(Species == "setosa")) ~ ., iris)
multi <- ranger(Species ~ ., data = iris)

test_that("ranger basics - regression", {
  expect_silent(safe_pred <- safe_predict(reg, mtcars))
  check_predict_basic(safe_pred, mtcars)
  pred <- predict(reg, mtcars)$predictions
  expect_same_content(safe_pred, pred)
})

test_that("ranger basics - binary", {
  expect_silent(safe_pred <- safe_predict(binary, iris))
  check_predict_basic(safe_pred, iris)
  pred <- predict(binary, iris)$predictions
  expect_same_content(safe_pred, pred)
})

test_that("ranger basics - multiclass", {
  expect_silent(safe_pred <- safe_predict(multi, iris))
  check_predict_basic(safe_pred, iris)
  pred <- predict(multi, iris)$predictions
  expect_same_content(safe_pred, pred)
})
